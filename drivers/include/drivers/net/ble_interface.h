...
{
	return ((struct ble_api *)iface)->gatt_create_service(mem,
...
{
	return ((struct ble_api *)iface)->gatt_add_characteristic(svc,
			uuid, uuid_len, chr);
}
...
static inline int ble_gatt_register_service(struct ble *iface,
		struct ble_gatt_service *svc)
{
	return ((struct ble_api *)iface)->gatt_register_service(svc);
}
...
{
	return ((struct ble_api *)iface)->gatt_response(ctx,
				data, datasize);
}
...
{
	return ((struct ble_api *)iface)->gatt_notify(iface, attr_handle,
				data, datasize);
}
